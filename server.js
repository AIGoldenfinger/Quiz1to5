...
app.get('/api/ip-info', async (req, res) => {
    ...
    try {
        const response = await axios.get(`http://api.ipstack.com/${clientIp}?access_key=${CONFIG.IPSTACK.API_KEY}`);
        const ip = ipaddr.process(response.data.ip).toString(); // Ensure IPv4
        res.json({ ip, country_name: response.data.country_name });
    } catch (error) {
